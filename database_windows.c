...
node *insert_DATA_tree(node *x, node *sample, int mode){ //全表\示での、二分探索木に挿入
    int judge;
    int i;
...
void treeprint(node *x, int updown){ //全表\示での二分探索木の表\示
    int i;
    char *name[3] = {"男性","女性","その他の性別"};
	if (x != NULL){
        if(updown == 1){
...
int show_DATA(node **DATA, int root) {
    ...
    if(nocount != 0){
        printf("\n管理者権限がないため、%d人のデータはパスワードにより表\示できませんでした。\n",nocount);
    }
    printf("終了します。\n");
}
...
int main() {
    ...
    while(1){
        printf("モードを選択してください\n");
        while(1){
            printf("0：終了　１：登録　２：変更　３：削除　４：検索　５：全表\示\n");
            if(scanf(" %d", &i) != 1){
                printf("入力が不適切です。\n");
                scanf("%*s");
            }
