...
int delete_DATA(FILE *fi, node **DATA){
	char mail[128];
	int val,handan;
	printf("削除したいメールアドレスを入力してください");
	scanf( "%s\n",mail);
	val = get_hash(mail);
	if(DATA[val] == NULL) {
		printf("存在していません\n");
		...
		if(DETA[val]->next == NULL){
		printf("%s %s %s %s %s %d %d %s %s %s %d年%d月%d日 %s %s\n",
                DATA[val]->name_ruby[0],DATA[val]->name_ruby[1],DATA[val]->name[0],DATA[val]->name[1],DATA[val]l->nickname,DATA[val]->postal[0],DATA[val]->postal[1],
                DATA[val]->address,DATA[val]->tell,DATA[val]->mail,DATA[val]->born[0],DATA[val]->born[1],DATA[val]->born[2],DATA[val]->job,name[DATA[val]->sex]);
